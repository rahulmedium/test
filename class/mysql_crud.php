...
/*
 * @Author Rahul Kumar <rk1448@gmail.com>
 * @Version 1.0
 * @Package Database
 */
...
class Database {
    ...
    private $db_name = "airwhizz"; // Change as required
    ...
    // Function to SELECT from the database
    public function select_single($table, $rows = '*', $join = null, $where = null, $order = null, $limit = null) {
        // Create query from the variables passed to the function
        $q = 'SELECT ' . $rows . ' FROM ' . $table;
        if ($join != null) {
            $q .= ' JOIN ' . $join;
    ...
    public function check_email($table,$email) {
        if ($this->tableExists($table)) {
            //echo "select * from $table where email='$email'";
             echo $q = "select * from users where email='$email'";
            $query = $this->myconn->query($q);
			//print_r($query);
			 if ($query) {
                 $this->numResults = $query->num_rows;
            ...
            //$password = md5($pass);
            //$query = mysqli_query($this->myconn, "select * from $table where email='$email'");
        } else {
            return false; // Table does not exist
        }
    }
    ...
    public function check_pass($table, $email, $pass) {
        if ($this->tableExists($table)) {
            $password = md5($pass);
    ...
    public function check_activate($table, $email) {
        if ($this->tableExists($table)) {
            $q = "select * from $table where email='$email' and status=1";
            $query = $this->myconn->query($q);
            if ($query) {
                $this->numResults = $query->num_rows;
                if ($this->numResults) {
